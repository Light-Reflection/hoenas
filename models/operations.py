...
# xception and mobileNetV1: depthwise separable conv
...
def drop_path(x, drop_prob, device):
  if drop_prob > 0.:
    ...
    mask = torch.cuda.FloatTensor(x.size(0), 1, 1, 1).bernoulli_(keep_prob).to(device)  #keep_prob 1, drop_prob 0
    x.div_(keep_prob)  # keep the same energy
    x.mul_(mask)
  return x